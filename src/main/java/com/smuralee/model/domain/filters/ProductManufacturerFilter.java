...
public class ProductManufacturerFilter extends LikeCriteriaFilter<Product> {
    ...
    public ProductManufacturerFilter(ViewModel request) {
        this.request = request;
    }

    @Override
    protected String getMatchPattern() {
        final String manufacturer = ((SearchRequest) this.request).productManufacturer();
        return StringConstants.LIKE_PREFIX + manufacturer.toLowerCase() +
                StringConstants.LIKE_PREFIX;
    }

    @Override
    protected Expression<String> getLikeAttribute(Root<Product> root) {
        return root.get("manufacturer").get("name");
    }
}
